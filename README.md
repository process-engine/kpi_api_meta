...
```
meta exec "any command" --exclude kpi_api_core,kpi_api_contracts
meta exec "any command" --include-only kpi_api_core,kpi_api_contracts
```
...
```
meta exec "git flow feature start my_feature" --include-only kpi_api,kpi_api_contracts
```

* Starts the feature "my_feature" in the modules
`kpi_api` and `kpi_api_contracts`

### 3.7 Publish a feature on multiple repositories

```
meta exec "git flow feature publish my_feature" --include-only kpi_api,kpi_api_contracts
```
